...
if [ "$1" != "nocmake" ] && [ "$1" != "install" ] ; then 
   ...
   if [ $COPYSOURCE ] ; then 
      ...
      MYCMAKEOPTS="-DCMAKE_C_FLAGS=-DOPENMP_NVPTX_COMPUTE_CAPABILITY=$NVPTXGPU -DCMAKE_CXX_FLAGS=-DOPENMP_NVPTX_COMPUTE_CAPABILITY=$NVPTXGPU -DCMAKE_BUILD_TYPE=$BUILDTYPE -DLIBOMPTARGET_NVPTX_DEBUG=1 -DLIBOMPTARGET_AMDGCN_DEBUG=1 -DCMAKE_INSTALL_PREFIX=$INSTALL_DIR -DBUILD_SHARED_LIBS=OFF -DCMAKE_C_COMPILER=$HCC2/bin/clang -DCMAKE_CXX_COMPILER=$HCC2/bin/clang++ -DLIBOMPTARGET_NVPTX_COMPUTE_CAPABILITY=$NVPTXGPU "
